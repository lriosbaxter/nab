...
def main(foreign_arguments):
    ...
    for index in range(sheet.nrows):
        ...
        print(props_node_cp)
        node_id = executor.submit(add_node_snmp,
                                  swis_connection,
                                  props_node,
                                  props_node_cp)
        discovered_interfaces = discover_interfaces(swis_connection, node_id)
        print(discovered_interfaces)
        results = NPMDevice.npm_invoke(swis_connection,
                                       'Cirrus.Nodes',
                                       'AddNodeToNCM',
                                       node_id)
